            ...
            try
            {
                ...
                for (var currentRowNumber = (sheet.FirstRowNum + 1); currentRowNumber <= sheet.LastRowNum; currentRowNumber++)
                {
                    ...
                    for (var currentColumnNumber = 0; currentColumnNumber < dt.Columns.Count; currentColumnNumber++)
                        dataRow[currentColumnNumber] = CryptoHelper.AES.EncryptToBase64String(row.GetCell(currentColumnNumber)?.ToString() ?? "", CryptoHelper.AES.Key, Encoding.UTF8, cipherMode: System.Security.Cryptography.CipherMode.ECB);
